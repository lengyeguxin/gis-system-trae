...
-- GIS警务指挥系统数据库初始化脚本
-- 基于04数据库设计说明书
-- 注意：此文件通过Spring Boot JDBC执行，不支持psql命令
-- =============================================
...
DROP TABLE IF EXISTS t_icon CASCADE;
...
CREATE TABLE t_alarm (
    ...
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    update_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT chk_alarm_level CHECK (alarm_level BETWEEN 1 AND 3),
    CONSTRAINT chk_alarm_status CHECK (status IN (0, 1)),
...
VALUES ('admin', '系统管理员', 'admin', '$2a$10$TNNdos6smVdV7/eh7/bMNeP6nb/93PoyrVf5soso9meWsd.RELbne');
...
-- 注意：更新时间自动更新功能已通过JPA的@PreUpdate注解实现，无需创建触发器
